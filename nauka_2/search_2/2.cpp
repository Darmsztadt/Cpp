...
int szukajNaiwny(string wzorzec, string tekst, int tab[])
{
    int licznik=0;
...
int main()
{
    ...
    int tab[txt.length()-wz.length()];
    int wynik = szukajNaiwny(wz, txt, tab);
    if (wynik == -1)
        cout << "Nie znaleziono\n";
    else for(auto e: tab)cout<<"Indeks znalezionego elementu: "<<tab[e]<<endl;
}